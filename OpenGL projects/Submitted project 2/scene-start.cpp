...
typedef struct {
    ...
    int texId;
    float texScale;
	float walkDist;	// distance that the gingerbreadman walks
	float walkRate;	// speed at which it walks
    bool removeObj; //Thashin part checks whether the object should be deleted or not
    float poseTime; //current frame
} SceneObject;
const int maxObjects = 1024; // Scenes with more than 1024 objects seem unlikely

float poseTimeIncrement = 0.04; //how much poseTime increments every time drawmesh is called
...
int flag; //Thashin: checks if menu is in use
int pattern = 0; //Thashin: used to determine pattern of walk if 0 walks in a straight line,if 1 walks in circular motion
//------------------------------------------------------------
...
{
	//Thashin: adding the option to delete the current object on screen
	if(!flag && currObject>3)
	{
     std::stringstream s;
     s << "Delete/Undelete Object " << currObject-2;
     glutAddMenuEntry((s.str()).c_str(), 100+currObject);
 }
}

static void addToSelectMenu(){
    if(!flag && currObject>3)
    {
     std::stringstream s;
     s << "Select Object " << currObject-2;
     glutAddMenuEntry((s.str()).c_str(), 200 + currObject);
 }
...
void loadMeshIfNotAlreadyLoaded(int meshNumber) {
    ...
    //**************
    //Thashin: Add option to delete to menu
    addToMenu();
    
    //Add item to select menu
    addToSelectMenu();
...
static void mouseClickOrScroll(int button, int state, int x, int y) {
    ...
    //prevPos is modified to get current mouse position
...
static void addObject(int id) {
	...
	//Init models with at least one bone
	if(id>55) {
		sceneObjs[nObjects].walkDist = 3;	
        sceneObjs[nObjects].walkRate = .01;				
	...
	//Whenever a new object is made, add to the select object menu
...
void init( void ) {
    ...
    sceneObjs[0].angles[0] = 90; // Rotate it.
    ...
    sceneObjs[1].brightness = 0.3; // The light's brightness is 5 times this (below).
    
    //Adding second light Sphere
...
void drawMesh(SceneObject sceneObj) {
    ...
    float poseTime = sceneObj.poseTime;
    // Set the model matrix - this should combine translation, rotation and scaling based on what's
    // in the sceneObj structure (see near the top of the program).
	float angle = sceneObj.angles[0] ;
	float angle2 = sceneObj.angles[1] ;
	float angle3 = sceneObj.angles[2] ;  
	mat4 model;
    model = Translate(sceneObj.loc) * Scale(sceneObj.scale)*RotateZ(-angle3) * RotateY(angle2)*RotateX(-angle) ;
   ...
   // Activate the VAO for a mesh, loading if needed.
    ...
    {          
        if(sceneObj.walkDist < 0.0) sceneObj.walkDist = 0.00001;    // Want to avoid negative/zero walk distance
        if(sceneObj.walkRate <= 0.0) sceneObj.walkRate = 0.0001;            // Want to avoid negative/zero walk speed
        vec4 distance;
        if(pattern ==0)
        {
            distance = RotateZ(-angle3) * RotateY(angle2) * RotateX(-angle) * vec4(0.0, 0.0, sceneObj.walkDist*cos(poseTime * sceneObj.walkRate), 0.0);
        }
        else
        {
            distance =RotateZ(-angle3) * RotateY(angle2) * RotateX(-angle) * vec4(cos(2*3.14*sceneObj.walkRate*poseTime)*sceneObj.walkDist/2, 0.0, sceneObj.walkDist*sin(2*3.14*sceneObj.walkRate*poseTime),0.0);
        }
        model = Translate(sceneObj.loc + distance)*model;
    }

    // Set the model-view matrix for the shaders
    glUniformMatrix4fv( modelViewU, 1, GL_TRUE, view * model );
    mat4 boneTransforms[nBones];     // was: mat4 boneTransforms[mesh->mNumBones];
    //Code supplied by Du in order to loop animation
    
    if (meshes[sceneObj.meshId]->mNumBones > 0) {
        while (poseTime >  scenes[sceneObj.meshId]->mAnimations[0]->mDuration)
            poseTime = poseTime - scenes[sceneObj.meshId]->mAnimations[0]->mDuration;
    }
...
void display( void ) {
    ...
    //Settings for second lighting sphere
    SceneObject lightObj2 = sceneObjs[2]; 
    vec4 lightPosition2 = view * lightObj2.loc ;
    
    //Changing lightPosition2 homogenous coordinate to 0.0
    ...
    glUniform3fv( glGetUniformLocation(shaderProgram, "Light1rgbBright2"), 1, lightObj2.rgb * lightObj2.brightness );

    
    
    for(int i=0; i<nObjects; i++) {
    	SceneObject so = sceneObjs[i];
		if(so.removeObj==false) //Thashin: Checks whether the object should be displayed or not, checking this here prevents unnecessary calculations
		{
	   ...
	   //Increments the pose time
       sceneObjs[i].poseTime += poseTimeIncrement;
   }
   glutSwapBuffers();
}
...
static void deleteMenu(int id)
{
  if (id >= 100)
  {
      //Thashin: an extremely trivial way of doing things
      if(sceneObjs[id-100].removeObj == false)
      {
          sceneObjs[id-100].removeObj = true;
      }
      else
      {
         sceneObjs[id-100].removeObj = false;
     }
     glutPostRedisplay();
 }
}
static void adjustWalkProperties(vec2 xy) {
	sceneObjs[currObject].walkDist+=xy[0];
	sceneObjs[currObject].walkRate+=xy[1];
}

static void animMenu(int id) {
	// Change walk distance and walk speed
	if(id == 202) {
		setToolCallbacks(adjustWalkProperties, mat2(0.001, 0, 0, 10.0),
         adjustWalkProperties, mat2(0.001, 0, 0, 10.0));
	}
    if(id == 203) {
        sceneObjs[currObject].walkDist = sceneObjs[currObject].walkDist * 2.0;
    }
    if(id == 204) {
        sceneObjs[currObject].walkDist = sceneObjs[currObject].walkDist / 2.0;
    }
	...
	if(id == 205) {
		sceneObjs[currObject].walkRate = sceneObjs[currObject].walkRate * 2.0;
	}
	if(id == 206) {
		sceneObjs[currObject].walkRate = sceneObjs[currObject].walkRate / 2.0;
	}
...
//Light callback functions
static void adjustAmbientDiffuse(vec2 ad){
  sceneObjs[toolObj].ambient += std::max(0.0f,ad[0]);
  sceneObjs[toolObj].diffuse += std::max(0.0f,ad[1]);

  
}

static void adjustSpecularShine(vec2 ss){
  sceneObjs[toolObj].specular += std::max(0.0f,ss[0]);
  sceneObjs[toolObj].shine += std::max(0.0f, ss[1]);
...
static void materialMenu(int id) {
    ...
    //Adjust ambient, diffuse, specular and shine light
    }else if(id ==20){
      toolObj = currObject;
      setToolCallbacks(adjustAmbientDiffuse, mat2(1,0,0,1), adjustSpecularShine, mat2(1,0,0,100) );
...
int modelFrameTotals[3] = {40,40,40};
...
static void mainmenu(int id) {
    ...
    //Duplication menu item
 if (id == 60 && currObject>=3)
 {
      //If the object to be duplicated is not a light
  if(sceneObjs[currObject].meshId != 55){

	//Duplicating the object
     sceneObjs[nObjects] = sceneObjs[currObject];


	//Incrementing the number of objects and updating currObject and toolObj
     toolObj = currObject = nObjects++;	

	//Offset the duplicated object a bit
     sceneObjs[toolObj].loc = sceneObjs[currObject-1].loc + vec4(0.5,0.0,0.5,0);


	//Add the duplicated object to menu
     addToMenu();

     glutPostRedisplay();
 }
}
    //Selecting object menu item
if (id >= 200){


    //Change the currObject and toolObj to be the one selected
    currObject = toolObj = id-200;
...
static void makeMenu() {
    ...
    int deleteMenuId = glutCreateMenu(deleteMenu);  
    glutAddMenuEntry("Delete/Undelete Background", 100);
    glutAddMenuEntry("Delete/Undelete Light 1", 101);
    glutAddMenuEntry("Delete/Undelete Light 2", 102);
    ...
    glutAddMenuEntry("Walk Distance/Walk Speed", 202);
     glutAddMenuEntry("Increase Walk Distance", 203);
      glutAddMenuEntry("Decrease Walk Distance", 204);
    glutAddMenuEntry("Increase Walk Speed", 205);
    glutAddMenuEntry("Decrease Walk Speed", 206);
    glutAddMenuEntry("Walk Pattern(Straight/Circular)", 207);
    ...
    glutAddSubMenu("Delete Objects",deleteMenuId);
    glutAddSubMenu("Animation Options", animMenuId);
    glutAddMenuEntry("EXIT", 99);
    glutAddMenuEntry("Duplicate Object", 60);
    glutAddMenuEntry("Select Object 1", 203);
    glutAddMenuEntry("Delete/Undelete Object 1", 103);
...
void reshape( int width, int height ) {
	...
	GLfloat nearDist = 0.45;
	GLfloat farDist = 100.0;
	GLfloat myview = 60.0;
	GLfloat ratio = (float)width/(float)height;
	if(width<height)
	{
		projection =  Scale(ratio)* Perspective(myview,ratio, -nearDist, farDist);
	}
	else
	{
		projection =  Perspective(myview,ratio, -nearDist, farDist);
	}
...
//Thashin: Checks to see if the menu is in use or not
...
int main( int argc, char* argv[] )
{
    ...
    makeMenu(); CheckError();
    //Thashin: Checks the status of the menu with the callback function "processMenuStatus"
